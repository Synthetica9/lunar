...
pub fn bench() -> u64 {
    ...
    let mut benches = BENCH_POSITIONS.iter();
        ...
        match status {
            ...
            search_thread::ThreadStatus::SearchFinished { depth, .. } => {
                if depth >= TARGET_DEPTH {
                    return Some(search_thread::ThreadCommand::StopSearch);
                }
            }
            search_thread::ThreadStatus::Idle => {
                ...
                let Some(fen) = benches.next() else {
    ...
    let mut thread = search_thread::ThreadData::new(0, tt_clone, Box::new(callback));
    thread.run();
